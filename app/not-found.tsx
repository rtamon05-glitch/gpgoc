...
export default function NotFound() {
  return (
    <Section tone="white" width="narrow">
      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-accent-ink">404</p>
      ...
      <ul className="mt-12 grid gap-2 border-t border-line pt-8 sm:grid-cols-2">
        {listCompanies().map((company) => (
          <li key={company.slug}>
            <a href={`/${company.slug}`} className="text-sm underline underline-offset-4 hover:text-accent-ink">
              {company.name}
            </a>
          </li>
        ))}
